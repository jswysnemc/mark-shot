...
完成选区后，使用移动工具调整范围。拖动边框、角点或选区内部时，也会显示已开启
的放大镜。方向键按 1 个图像像素调整指针所在的边框或角点；指针离开调整手柄时，
方向键移动整个选区。按住 Shift 时每次调整 10 像素。拖动过程中也能使用方向键，
关闭放大镜后仍可微调已有选区。
...
独立 OCR 结果窗口默认使用普通窗口，niri 可以通过 `niri msg pick-window` 识别它，
也可以应用窗口规则。拖动标题栏或使用窗口管理器的移动操作即可调整位置。
置顶按钮单独保存 `ocrResultWindow.alwaysOnTop`，不再继承钉图设置。在通过
layer-shell 实现置顶的合成器上，开启置顶后窗口会使用图层表面；关闭置顶即可
恢复普通窗口管理。
由于 Wayland 不提供普通窗口的全局位置，切换到 layer-shell 置顶模式时，OCR
面板会在当前屏幕居中。置顶状态下仍可拖动标题栏。
...
从托盘菜单选择 **截图历史**，执行 `mark-shot --history`，或在截图覆盖层按
`Ctrl+H` 打开历史窗口。从覆盖层打开历史会结束当前截图。截图编辑器成功执行
复制、保存、另存为或钉图后，会记录含有标注和导出效果的实际图像，独立于选区
矩形历史。

选择缩略图即可预览，再执行复制、编辑、钉图、另存为或删除。**清空历史**需要
确认，只删除历史副本，保留另存的文件。直接从历史复制、钉图或保存不会重复记录；
编辑历史图片并导出后，会保存更新后的图像。

自动记录默认开启，可在历史窗口关闭。默认保留最近 50 张截图，总容量不超过
256 MiB，超出限制时删除最早的记录。`captureHistory.limit` 可设为 1 至 200。
历史图片保存在应用数据目录的 `capture-history` 子目录中，重启后仍可查看；
Linux 默认位置为 `~/.local/share/mark-shot/capture-history`，遵循 XDG 数据目录
设置。仅上传、仅 OCR、显示器快照面板和无界面截图不会新增历史。
