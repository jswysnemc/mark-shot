...
开启 **设置 → 存储 → 截图导出外观 → Mac 风格外框** 后，保存 / 复制 / 上传的图片会带上透明
内边距、圆角与柔和阴影。
...
独立 OCR 结果窗口默认显示可编辑的识别原文。顶部“文字 / 原图”切换同一个内容区域，
原图按窗口大小等比显示。切回文字后保留编辑、撤销历史、滚动位置和分栏大小。
点击顶部“翻译”后，窗口展开译文区，目标语言和翻译操作位于编辑器上方，并使用
已保存的语言开始翻译。两边均可单独复制，复制反馈显示在
对应按钮上，编辑后会出现撤销入口。宽窗口并排显示原文和译文，窄窗口上下排列；
拖动分隔条可以调整两边的空间，右下角可以调整窗口大小。

翻译期间可以取消；“收起译文”也会取消正在进行的请求，并保留已有译文。
原文或目标语言改变后，窗口会提示重新翻译。
`Ctrl+Enter` 翻译，`Ctrl+Shift+C` 复制当前编辑区全文，
`Ctrl+C` 仍只复制选中文字，`Esc` 关闭窗口。窗口随界面设置使用浅色或深色主题。

OCR 结果窗口默认使用普通窗口，niri 可以通过 `niri msg pick-window` 识别它，
...
从托盘菜单或已配置的录制快捷键打开准备窗口。在标题旁选择视频或 GIF，再选择
显示器或区域。区域模式的主按钮为“选择区域”，整屏模式为“开始录制”。视频模式
开启“录制音频”后，才显示音频输入设备。

“保存位置”显示输出文件名，展开后可以修改路径。“录制选项”显示当前帧率、格式和
视频质量，展开后可以调整详细参数、倒计时和采集后端。窗口随展开内容自动调整高度，
屏幕空间不足时可滚动查看选项，底部操作保持可见。切换视频与 GIF 会保留
各自帧率及音频选择，GIF 隐藏音频和视频专用选项。录制中的紧凑控制条显示时长及
“录制中 / 已暂停”状态，并提供暂停、继续和停止操作。

---

## 9. 配置与后端

从托盘菜单打开“设置”。各分组的标签列和输入起点保持对齐，数字和快捷键字段采用
紧凑宽度，开关说明紧邻勾选框。窄窗口使用顶部分类选择器，字段标签移到输入框上方，
长标签和开关说明自动换行。高级分组在当前页面展开，收起
后保留输入。“保存”应用修改并保持窗口打开，结果显示在底部。“撤销修改”恢复最近
一次保存的配置，并保留当前所在页面。在单行文本输入框中按 `Enter` 可以保存修改，窗口
保持打开。将选项改回已保存的值后，未保存提示会自动消失。
