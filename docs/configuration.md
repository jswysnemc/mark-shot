...
| `pinnedWindow.autoOcr` | Boolean | `false` | Controls whether a pinned sticker window starts OCR text recognition in the background immediately on creation. If disabled, OCR runs on demand on the first text-selection gesture or when Copy Image Text or Translate is chosen. The first selection gesture is preserved until text positions are available. Alias: `pinned`, `pin`. |
